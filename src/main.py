...
PATH_ALGORITHM = os.environ.get('PATH_ALGORITHM', 'autotruck-path')
...
agent_data = {          
                'name': VEHICLE_NAME,
                'pose': {'x': X0, 'y':Y0, 'orientations':[ORIENTATION_0, 0]},
                'geometry': GEOMETRY,
                'factsheet': GEOMETRY,
...
def my_cancel_assignm_callback(*args): return cancel_assignm_callback(vehi_state_ros, agentConnector, *args )
...
def my_assignment_callback(ch, method, properties, inst_assignment_msg): 
    ...
    assignment_execution_thread = Thread(target=assignment_execution_local_simulator, args=(inst_assignment_msg, PATH_ALGORITHM, *operation_topics))
